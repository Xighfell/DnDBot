...
async def on_message(message):
    ...
    # Resolve player or party name
        ...
        queue = turn_order[channel_id]  # Still get the queue for turn rotation below if needed

    # Helper function to determine outcome based on d20 roll
    def roll_outcome(d20_roll):
        if d20_roll <= 7:
            return "fail"
        elif d20_roll <= 14:
            return "moderate"
    ...
    # Find matching action key (async call)
    action_key = await find_action_key_with_ai(action)

    if action_key and input_name != "party":
        roll = random.randint(1, 20)
        outcome = roll_outcome(roll)

        # 30% chance to fallback to AI instead of flavor text
        use_ai = random.random() < 0.3

        if not use_ai:
            flavor_section = flavor_data.get(action_key, {})
            flavor_texts = flavor_section.get("flavor_text", {})
            flavor_options = flavor_texts.get(outcome, [])
            if flavor_options:
                flavor_text = random.choice(flavor_options).replace("{player}", resolved_name.capitalize())
                await message.channel.send(f"🎲 Roll: {roll} → {outcome.capitalize()}\n{flavor_text}")
